...
try {
    ...
    if ($ok) {
        ...
        if (($cliente['plan_tipo'] ?? '') === 'tiquetera' && !empty($cliente['tiquetera_factura_id'])) {
            ...
            $logLine = date('Y-m-d H:i:s') . " cliente=$cliente_id factura=$facturaActiva total=$totalAhora limite=$limiteEntradas\n";
            file_put_contents(BASE_PATH . '/tiq_debug.log', $logLine, FILE_APPEND);

            if ($totalAhora >= $limiteEntradas) {
                file_put_contents(BASE_PATH . '/tiq_debug.log', date('H:i:s') . " AGOTADA — enviando WA\n", FILE_APPEND);
                ...
                if ($clienteWs && !empty($clienteWs['telefono'])) {
                    ...
                    $waEndpoint = rtrim(WA_API_URL, '/') . '/send';
                    $waPayload  = json_encode([
                        'phonenumber' => $clienteWs['dialCode'] . $clienteWs['telefono'],
                        'text'        => $mensaje,
                    ], JSON_UNESCAPED_UNICODE);

                    file_put_contents(BASE_PATH . '/tiq_debug.log',
                        date('H:i:s') . " endpoint=$waEndpoint payload=$waPayload apiKey=" . (empty($api_ws) ? 'VACIO' : 'OK') . "\n",
                        FILE_APPEND);

                    $waCh = curl_init();
                    curl_setopt_array($waCh, [
                        CURLOPT_URL            => $waEndpoint,
                    ...
                    file_put_contents(BASE_PATH . '/tiq_debug.log',
                        date('H:i:s') . " httpCode=$waCode error=$waError response=$waResponse\n",
                        FILE_APPEND);
                    ...
                    file_put_contents(BASE_PATH . '/tiq_debug.log', date('H:i:s') . " SKIP — cliente sin teléfono\n", FILE_APPEND);
                }
            }
        }
