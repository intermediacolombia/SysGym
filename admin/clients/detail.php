...
<script>
$(document).ready(function(){
  $('#asistencias-tab').on('shown.bs.tab', function(){
    if (!$.fn.DataTable.isDataTable('#asistencias-table')) {
      $('#asistencias-table').DataTable({
        ajax: {
          url : 'get_asistencias.php',
        ...
        columns: [
          ...
          { data: 'fecha' },
          {
            ...
            render: function(data, type) {
              if (type === 'display') {
                var p = data.split(':');
                var hh = parseInt(p[0], 10), suf = ' am';
                if (hh === 0) hh = 12;
                else if (hh >= 12){ suf = ' pm'; if (hh > 12) hh -= 12; }
                return '<span class="hora-pill">' + hh + ':' + p[1] + suf + '</span>';
              }
              return data;
            }
          }
...
$(document).ready(function(){
  ...
  function cargarEstadisticas() {
    ...
    $.ajax({
      ...
      success: function(json) {
        ...
        poblarMeses(data);
        calcularYMostrar(data, document.getElementById('att-month-select').value);
        document.getElementById('att-month-select').onchange = function(){
          calcularYMostrar(data, this.value);
        };
      },
      error: function(xhr) {
        console.error('Error stats:', xhr.status, xhr.responseText);
      }
    });
  }
  ...
  function diasHabiles(year, month) {
    var d = new Date(year, month-1, 1), n = 0;
  ...
  function rachas(sortedDesc) {
    if (!sortedDesc.length) return {actual:0, mejor:0};
    var set = {}, asc = sortedDesc.slice().reverse();
    sortedDesc.forEach(function(f){ set[f]=true; });
    var mejor=1, curr=1;
    for (var i=1; i<asc.length; i++){
      var diff = (new Date(asc[i]) - new Date(asc[i-1])) / 86400000;
      if (diff===1||(diff===2&&new Date(asc[i-1]).getDay()===6)){ curr++; if(curr>mejor) mejor=curr; }
      else curr=1;
    }
    var racha=0, d=new Date(); d.setHours(0,0,0,0);
    for (var j=0; j<90; j++){
      if (d.getDay()>=1&&d.getDay()<=6){
        if (set[d.toISOString().slice(0,10)]) racha++;
        else break;
      }
      d.setDate(d.getDate()-1);
    }
  ...
  function calcularYMostrar(data, mes) {
    if (!mes) return;
    var p  = mes.split('-');
    var y  = parseInt(p[0]), m = parseInt(p[1]);
    var fi = data.filter(function(r){ return r.fecha.indexOf(mes)===0; });
    var total   = fi.length;
    var habiles = diasHabiles(y, m);
    var faltas  = Math.max(0, habiles - total);
    var pct     = habiles ? Math.round(total/habiles*100) : 0;

    var uniq={}, fechas=[];
    data.forEach(function(r){ var f=r.fecha.slice(0,10); if(!uniq[f]){uniq[f]=true;fechas.push(f);} });
    fechas.sort().reverse();
    var r = rachas(fechas);
    ...
    document.getElementById('stat-racha').textContent       = r.actual + ' días';
    document.getElementById('stat-mejor-racha').textContent = r.mejor  + ' días';
    ...
    /* Heatmap */
    var dc={};
    DIAS_NORM.forEach(function(d){ dc[d]=0; });
    fi.forEach(function(r){ var n=norm(r.dia_semana); if(typeof dc[n]!=='undefined') dc[n]++; });
    var mx=1;
    DIAS_NORM.forEach(function(d){ if(dc[d]>mx) mx=dc[d]; });
    var html='';
    DIAS.forEach(function(d,i){
      var c=dc[DIAS_NORM[i]]||0, rat=c/mx;
