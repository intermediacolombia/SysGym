...
<script>
	  // credito_max.js
// Requiere jQuery y Bootstrap (para el evento 'shown.bs.modal')

(function ($) {
  "use strict";

  function leerNumero(selector, def) {
    var v = parseFloat($(selector).val());
    return isNaN(v) ? def : v;
  }

  function leerEntero(selector, def) {
    var v = parseInt($(selector).val(), 10);
    return isNaN(v) ? def : v;
  }

  function calcularTotal() {
    // #plan_precio debe contener el precio del plan (input hidden)
    var planPrecio = leerNumero("#plan_precio", 0);
    var periodos   = Math.max(1, leerEntero("#periodos_pagar", 1));
    return planPrecio * periodos;
  }

  function aplicarMaxValorPagado() {
    // Solo aplicar límite si está seleccionado "Crédito"
    if (!$("#credit_check").is(":checked")) {
      $("#valor_pagado").removeAttr("max");
      return;
    }

    var total   = calcularTotal();
    var maximo  = Math.max(total - 1, 0); // $1 menos que el total

    $("#valor_pagado").attr("max", maximo);

    // Asegurar que el valor actual no exceda el máximo
    var actual = leerNumero("#valor_pagado", 0);
    if (actual > maximo) {
      $("#valor_pagado").val(maximo);
    }
  }

  // Recalcular límite cuando:
  // - se marca/desmarca Crédito
  // - cambia la cantidad de períodos
  // - se abre el modal de pago
  $(document).on("change input", "#credit_check, #periodos_pagar", aplicarMaxValorPagado);
  $(document).on("shown.bs.modal", "#paymentModal", aplicarMaxValorPagado);

  // Opcional: si el usuario escribe en valor_pagado por encima del max, recortar
  $(document).on("input", "#valor_pagado", function () {
    var maxAttr = $(this).attr("max");
    if (typeof maxAttr !== "undefined") {
      var max = parseFloat(maxAttr);
      var val = leerNumero(this, 0);
...
?>
<script>
// === JS PARA PLANES VENCIDOS (Cliente ingresa valores manuales) ===
$(document).ready(function(){
  const originalTotal = <?php echo isset($planInfo['precio']) ? floatval($planInfo['precio']) : 0; ?>;
  const paymentModal = new bootstrap.Modal(document.getElementById('paymentModal'), { keyboard: false });
  let processingPayment = false;

  // Calcular total según períodos seleccionados
  function calcularTotal() {
    const planPrecio = parseFloat($("#plan_precio").val()) || originalTotal;
    const periodos = parseInt($("#periodos_pagar").val()) || 1;
    return planPrecio * periodos;
  }

  // Evitar que la suma de pagos divididos exceda el total permitido
  function limitarPagosDivididos() {
    const totalPermitido = calcularTotal();
    ...
    if (total > totalPermitido) {
      alert("La suma de los pagos no puede exceder el total a pagar ($" + totalPermitido.toLocaleString('es-CO') + ")");
      if (document.activeElement.id === "first_payment_value") {
        $("#first_payment_value").val(first - (total - totalPermitido));
      } else {
        $("#second_payment_value").val(second - (total - totalPermitido));
      }
    }
  }
  ...
  $("#btnPago").click(function(){
    ...
    $("#valorRestanteDisplay").text("Valor Restante: $" + total.toLocaleString('es-CO'));
    $("#originalTotalDisplay").text("$" + total.toLocaleString('es-CO'));
    paymentModal.show();
  });
  ...
  $("#split_payment_check").change(function(){
    ...
    $("#secondPaymentWrapper, #firstPaymentValueWrapper").toggle(checked);
    if (!checked) {
  ...
  $("#confirmPaymentBtn").off('click').on('click', function(){
    ...
    const planPrecio = parseFloat($("#plan_precio").val()) || originalTotal;
    let periodos = parseInt($("#periodos_pagar").val()) || 1;
    ...
    let valorPagado = creditSelected ? $("#valor_pagado").val() : totalPagar;
    ...
    if (splitPayment && (firstValue + secondValue) > totalPagar) {
      Swal.fire('Error', 'La suma de los pagos no puede superar el total.', 'error');
      processingPayment = false;
      return;
    }

    if (!paymentMethod) { alert("Seleccione un medio de pago"); processingPayment = false; return; }
    if (paymentMethod === 'Transferencia' && !bank) { alert("Seleccione un banco"); processingPayment = false; return; }
    ...
    Swal.fire({
      title: "Confirmar Pago",
      html: `
        <p><b>Períodos a pagar:</b> ${periodos}</p>
        <p><b>Total a pagar:</b> $${totalPagar.toLocaleString('es-CO')}</p>
      `,
      icon: "question",
      showCancelButton: true,
      ...
      if (r.isConfirmed) {
        let today = new Date();
        let pago_plan = today.toISOString().slice(0,10);
        ...
        venc.setMonth(venc.getMonth() + (planFrecuencia * periodos));
        venc.setDate(venc.getDate() + (planDias * periodos) - 1);
        let vencimiento_plan = venc.toISOString().slice(0,10);
        ...
        $.ajax({
          ...
          data: {
            ...
            periodos_pagar: periodos
          },
          dataType: 'json',
          success: function(res){
            if(res.status === 'success'){
              Swal.fire('Éxito', 'Pago registrado correctamente.', 'success');
              $("#fecha_pago_display").text(res.pago_plan);
              $("#fecha_vencimiento_display").text(res.vencimiento_plan);
...
?>
<script>
// === JS PARA PLANES ACTIVOS (Cliente ingresa montos manuales) ===
$(document).ready(function(){
  const originalTotal = <?php echo isset($planInfo['precio']) ? floatval($planInfo['precio']) : 0; ?>;
  const paymentModal = new bootstrap.Modal(document.getElementById('paymentModal'), { keyboard: false });
  let processingPayment = false;
  ...
  function limitarPagosDivididos() {
    ...
    const total = first + second;
    if (total > totalPermitido) {
      alert("La suma de los pagos no puede exceder el total permitido ($" + totalPermitido.toLocaleString('es-CO') + ")");
      if (document.activeElement.id === "first_payment_value") {
        $("#first_payment_value").val(first - (total - totalPermitido));
      } else {
        $("#second_payment_value").val(second - (total - totalPermitido));
