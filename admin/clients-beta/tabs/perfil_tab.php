  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
		  ...
		  <!-- NUEVO: Selección de períodos -->
<div class="mb-3">
  <label for="periodos_pagar" class="form-label">Períodos a Pagar</label>
  <input type="number" id="periodos_pagar" class="form-control" value="1" min="1" required>
</div>

		  
		  
        <p id="valorPagar">
  Valor a Pagar: <strong id="totalPagarDisplay">$<?php echo isset($planInfo['precio']) ? number_format($planInfo['precio'], 0, ',', '.') : '0'; ?></strong>
</p>
<input type="hidden" id="plan_precio" value="<?= isset($planInfo['precio']) ? $planInfo['precio'] : 0 ?>">
...
document.addEventListener('DOMContentLoaded', function() {
    const precioPlan = parseFloat(document.getElementById('plan_precio').value || 0);
    const periodosInput = document.getElementById('periodos_pagar');
    const totalDisplay = document.getElementById('totalPagarDisplay');

    function actualizarTotal() {
        const periodos = parseInt(periodosInput.value) || 1;
        const total = precioPlan * periodos;
        totalDisplay.textContent = '$' + total.toLocaleString('es-CO');
    }

    periodosInput.addEventListener('input', actualizarTotal);
    actualizarTotal();
});
</script>
