...
*/
$mensaje = str_replace(
    array("{nombres}", "{apellidos}", "{fecha_pago}", "{valor}"),
    array($clientInfo['nombres'], $clientInfo['apellidos'], $hoy, $valor),
    $mensaje
);

/*
  Construimos el array con los datos que se enviarÃ¡n a la API.
  AsegÃºrate de que $cp_dialCode, $cp_telefono, $url y $formulario_id
  estÃ©n definidos en el script que hace el include de este archivo, si los necesitas.
*/
$data = array(
    'phonenumber' => $clientInfo['dialCode'] . $clientInfo['telefono'],
	//'phonenumber' => '573172998776',
